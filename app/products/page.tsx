...
function ProductsContent() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [query, selectedCategory, selectedPrice, sort]);
    ...
    <div className="min-h-screen bg-cream">
        ...
        {activeFilterCount > 0 && (
          <div className="px-4 sm:px-6 lg:px-8 pb-2 flex gap-2 overflow-x-auto">
            {selectedCategory !== "all" && (
              <button
                ...
                className="flex items-center gap-1 px-3 py-1 bg-terracotta/10 text-terracotta
                           rounded-full text-xs font-medium whitespace-nowrap active:bg-terracotta/20"
              >
            ...
            <button
              ...
              className="px-3 py-1 text-xs text-taupe-dark active:text-terracotta whitespace-nowrap"
            >
              Clear all
            </button>
          </div>
          ...
          <aside className="hidden lg:block w-56 shrink-0 sticky top-40">
            <div className="bg-white rounded-2xl border border-cream-darker border-l-[3px] border-l-brown-light/40 p-5 shadow-sm">
              ...
              {activeFilterCount > 0 && (
                <button
                  onClick={clearFilters}
                  className="mt-4 w-full text-center text-xs text-terracotta
                             active:text-gold transition-colors duration-150 underline"
                >
                  Clear all filters
                </button>
              )}
          ...
          <div className="flex-1 min-w-0">
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-20">
                ...
                <button
                  onClick={clearFilters}
                  className="text-sm text-terracotta underline active:text-gold"
                >
                  Clear filters
                </button>
              </div>
            ) : (
              /* 2-col mobile, 2 tablet, 3 desktop */
      ...
      >
        <div className="space-y-6 pb-2">
          {/* Category */}
          <div>
            <h3 className="text-xs uppercase tracking-[0.15em] text-taupe-dark font-medium mb-3">
              Category
            </h3>
          ...
          <button
            ...
            className="w-full h-14 rounded-2xl bg-terracotta text-cream font-semibold text-base
                       active:bg-terracotta-dark active:scale-[0.98] transition-all duration-150"
            style={{ touchAction: "manipulation" }}
          >
          ...
          {activeFilterCount > 0 && (
            <button
              ...
              className="w-full text-center text-sm text-taupe-dark active:text-terracotta underline"
            >
              Clear all filters
            </button>
          )}
