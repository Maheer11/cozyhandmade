...
const tabs = [
  {
    ...
    label: "Home",
    icon: (active: boolean) => (
      <svg className={`w-6 h-6 ${active ? "fill-terracotta" : "fill-none stroke-taupe-dark"}`}
  ...
  },
  {
    ...
    label: "Search",
    icon: (active: boolean) => (
      <svg className={`w-6 h-6 ${active ? "stroke-terracotta" : "stroke-taupe-dark"} fill-none`}
           viewBox="0 0 24 24" strokeWidth={1.8}>
      ...
      <div className="flex items-stretch h-14">
        {tabs.map(({ href, label, icon }) => {
          ...
          return (
            ...
            >
              {icon(active, isCart ? itemCount : undefined)}
              <span
